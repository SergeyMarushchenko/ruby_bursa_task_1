class LibraryManager
  ...
  def penalty price, issue_datetime
    ...
    overdue_time = ((Time.now.to_f - issue_datetime.to_f)/3600).ceil > 0 ? (overdue_time*price*0.001).round(0) : 0
  ...
  def could_meet_each_other? year_of_birth_first, year_of_death_first, year_of_birth_second, year_of_death_second
    ...
    life_time1 = year_of_birth_first..year_of_death_first 
    
    life_time1.include? (year_of_birth_second || year_of_death_second) 
  ...
  def days_to_buy price
    number_of_days = (price/(0.001*price*24)).ceil
    # решение пишем тут
  end
...
+    }
  ...
  def penalty_to_finish price, issue_datetime, pages_quantity, current_page, reading_speed
    ...
    seconds_to_complete = ((( pages_quantity - current_page ) / reading_speed)*3600).ceil
    complete_time = (Time.now.to_f + seconds_to_complete).round(0)
    time_diff = ((complete_time - issue_datetime.to_f)/3600).ceil > 0 ? time_diff*price*0.001 : 0
